...
import {
    ...
    saveIssues,
    addNewIssue,
    getFilteredIssues as getFilteredIssuesHelper
} from '../../lib/issueHelpers';
import { AnimatedText } from "../ui/animated-underline-text-one";
import { getFilteredIssues } from "../../lib/issueHelpers";
    ...
    return (
        <aside className="w-64 flex-shrink-0 bg-black/30 border-r border-blue-500/20 p-6 flex flex-col">
            ...
            <nav className="flex flex-col space-y-2">
                {navItems.map(item => (<button key={item.id} onClick={() => setMainTab(item.id)} className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors duration-200 ${mainTab === item.id ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:bg-white/10 hover:text-white'}`}>{item.icon}<span className="font-semibold">{item.label}</span></button>))}
            </nav>
    ...
    return (
        <div className="bg-black/50 rounded-2xl overflow-hidden border border-blue-500/20 shadow-lg flex flex-col">
            <div className="p-6 flex flex-col flex-grow">
                 ...
                 <div className="space-y-3 text-gray-400 text-sm mb-4"><div className="flex items-center"><CalendarIcon className="h-4 w-4 mr-3 shrink-0" /><span>{formatDate(issue.created_at)}</span></div>{issue.location && (<div className="flex items-center"><MapPinIcon className="h-4 w-4 mr-3 shrink-0" /><span>{issue.location}</span></div>)}</div>
                 <p className="text-gray-400 text-sm mb-5 flex-grow">{issue.description}</p>
                 <div className="flex flex-wrap gap-2 pt-5 border-t border-white/10">{tags.map((tag, i) => (<span key={i} className="bg-blue-900/50 text-blue-300 text-xs font-semibold px-3 py-1 rounded-full">{tag}</span>))}</div>
                 <div className="mt-4 pt-4 border-t border-white/10">
                     <button onClick={() => onUpvote(issue.id)} disabled={hasVoted} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 text-sm font-medium ${hasVoted ? 'bg-green-500/20 text-green-400 cursor-not-allowed' : 'bg-blue-600/20 hover:bg-blue-600/40 text-blue-300'}`}>
                         {hasVoted ? <CheckIcon className="w-4 h-4" /> : <UpvoteIcon className="w-4 h-4" />}
                         <span>{hasVoted ? t('issue_card.voted') : t('issue_card.upvote')}</span>
                         <span className={`px-2 py-1 rounded-full text-xs ${hasVoted ? 'bg-green-900/50 text-green-300' : 'bg-blue-900/50 text-blue-300'}`}>{issue.upvotes || 0}</span>
                     </button>
                 </div>
            </div>
        </div>
...
const LeaderboardPage: React.FC<LeaderboardPageProps> = ({ users, currentUserId }) => {
    ...
    const getRankColor = (rank: number) => {
        if (rank === 1) return 'bg-yellow-500 text-yellow-900 border-yellow-400';
        if (rank === 2) return 'bg-gray-400 text-gray-900 border-gray-300';
        if (rank === 3) return 'bg-orange-600 text-orange-100 border-orange-500';
        return 'bg-gray-700 text-gray-200 border-gray-600';
    };
    return (<div className="max-w-3xl mx-auto"><div className="space-y-4">{sortedUsers.map((user, index) => (<div key={user.id} className={`flex items-center p-4 rounded-lg transition-all ${user.id === currentUserId ? 'bg-blue-900/50 ring-2 ring-blue-400' : 'bg-black/30 border border-blue-500/10'}`}><div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg mr-4 border-2 ${getRankColor(index + 1)}`}>{index + 1}</div><div className="flex-grow"><p className="font-semibold text-white text-lg">{user.name}</p>{user.id === currentUserId && <p className="text-sm text-blue-400">{t('misc.you')}</p>}</div><div className="flex items-center gap-2 text-yellow-300 font-bold text-xl"><TrophyIcon className="w-6 h-6 text-yellow-400" />{user.points.toLocaleString('en-IN')}</div></div>))}</div></div>);
};
const ProfilePage: React.FC<ProfilePageProps> = ({ user }) => {
    const { t } = useTranslation();
    if (!user) return (<div className="text-center py-16 px-6 bg-black/30 rounded-lg"><h3 className="text-xl font-semibold text-white">{t('profile_page.not_found_title')}</h3><p className="text-gray-500 mt-2">{t('profile_page.not_found_subtitle')}</p></div>);
    return (<div className="max-w-4xl mx-auto"><div className="bg-black/30 rounded-lg border border-blue-500/20 backdrop-blur-sm p-8 mb-8 text-center"><h3 className="text-4xl font-bold text-white">{user.name}</h3><p className="text-lg text-gray-400">{t('profile_page.contributor_title')}</p><div className="mt-4 inline-flex items-center gap-2 text-yellow-300 font-bold text-2xl bg-yellow-600/20 px-4 py-2 rounded-lg"><TrophyIcon className="w-6 h-6 text-yellow-400" />{user.points.toLocaleString('en-IN')} {t('header.points')}</div></div><div className="bg-black/30 rounded-lg border border-blue-500/20 backdrop-blur-sm p-8"><h4 className="text-2xl font-bold text-white mb-6">{t('profile_page.achievements_title')}</h4><p className="text-gray-400">{t('profile_page.achievements_soon')}</p></div></div>);
};
...
export default function App() {
            ...
            if (!currentIssues || currentIssues.length === 0) {
                const initialMockIssues = [
                    ...
                    },
                    {
                        id: 3,
                        user_id: 'user-789',
                        title: "Streetlight Malfunction in Hauz Khas",
                        description: "Three streetlights not working on the main road near park entrance. Safety concern for evening commuters.",
                        created_at: "2025-09-15T08:00:00Z",
                        location: "Hauz Khas",
                        tags: "infrastructure,safety,lighting",
                        upvotes: 28,
                        status: 'pending' as const
                    }
                ];
                console.log('Creating initial mock issues:', initialMockIssues);
                saveIssues(initialMockIssues);
                setIssues(initialMockIssues);
            }

            // Add a test issue for the current user if logged in
            if (loggedInUser && currentIssues) {
                const userHasIssues = currentIssues.some((issue: Issue) => issue.user_id === loggedInUser.id);
                if (!userHasIssues) {
                    const newIssue = {
                        id: currentIssues.length + 1,
                        user_id: loggedInUser.id,
                        title: "Test Issue",
                        description: "This is a test issue for the new user",
                    ...
                    console.log('Adding test issue for current user:', newIssue);
                    saveIssues(updatedIssues);
                    setIssues(updatedIssues);
                }
            }
            
            console.log('Setting issues state:', currentIssues);
    ...
    const handleUpvote = (issueId: number) => {
        ...
        if (!issueToUpdate || !authUser) return;

        // Add the vote and update points
        addVote(issueId, authUser.id);
    ...
    const getDisplayedIssues = () => {
        ...
        if (mainTab === 'my_issues') {
            ...
            return getFilteredIssues({ userId: currentUserId });
        }
